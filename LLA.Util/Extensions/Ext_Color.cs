...
namespace LLA.Core
{
    ...
    public static class Ext_Color
    {
        ...
        public static double[] RgBtoHsl(Color color)
        {
            ...
            // hue
            if (SystemMath.Abs(max - min) < Ext_Color.tolerance)
                h = 0D; // undefined
            else if ((SystemMath.Abs(max - r) < Ext_Color.tolerance)
                    && (g >= b))
                h = (60D * (g - b)) / (max - min);
            else if ((SystemMath.Abs(max - r) < Ext_Color.tolerance)
                    && (g < b))
                h = ((60D * (g - b)) / (max - min)) + 360D;
            else if (SystemMath.Abs(max - g) < Ext_Color.tolerance)
                h = ((60D * (b - r)) / (max - min)) + 120D;
            else if (SystemMath.Abs(max - b) < Ext_Color.tolerance)
            ...
            // saturation
            if ((SystemMath.Abs(l) < Ext_Color.tolerance)
                    || (SystemMath.Abs(max - min) < Ext_Color.tolerance))
                s = 0D;
            else if ((0D < l)
                    && (l <= .5D))
                s = (max - min) / (max + min);
        ...
        public static Color HsLtoRgb(double h, double s, double l, int a = 255)
        {
            ...
            if (SystemMath.Abs(s) < Ext_Color.tolerance)
            {
        ...
        public static double[] RgBtoHsb(Color color)
        {
            ...
            double h = 0D;
            if ((SystemMath.Abs(max - r) < Ext_Color.tolerance)
                    && (g >= b))
                h = (60D * (g - b)) / (max - min);
            else if ((SystemMath.Abs(max - r) < Ext_Color.tolerance)
                    && (g < b))
            ...
            double s = SystemMath.Abs(max) < Ext_Color.tolerance
                    ? 0D
                    : 1D - (min / max);
                ...
                byte? outputAlpha = null)
        {
            double[] hsl = colorTransformMode == ColorTransformMode.Hsl
                    ? Ext_Color.RgBtoHsl(color)
                    : Ext_Color.RgBtoHsb(color);
            if ((SystemMath.Abs(hsl[2]) < Ext_Color.tolerance)
            ...
            return colorTransformMode == ColorTransformMode.Hsl
                    ? Ext_Color.HsLtoRgb(hsl[0], hsl[1], hsl[2], outputAlpha ?? color.A)
                    : Ext_Color.HsBtoRgb(hsl[0], hsl[1], hsl[2], outputAlpha ?? color.A);
        }
                ...
                byte? outputAlpha = null)
        {
            double[] hsl = colorTransformMode == ColorTransformMode.Hsl
                    ? Ext_Color.RgBtoHsl(color)
                    : Ext_Color.RgBtoHsb(color);
            if ((SystemMath.Abs(hsl[1]) < Ext_Color.tolerance)
        ...
        public static Color FromHSLA(Int32 hue, Int32 saturation, Int32 value, Int32 alpha)
        {
